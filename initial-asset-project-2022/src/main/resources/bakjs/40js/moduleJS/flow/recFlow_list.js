{
	...
	buttons: [
		...
		},
		{
			...
			click: function () {
				...
				this.$confirm('您确定重置选定的记录行（流程文档实例）吗? \n' +titles , '重置', {
		        	...
		        	this.post(`${this.BASEURL}/instance/flow/flowinstancemonitor/resume.do`, params, (isSuccess, res) => {
	        	  		if (res.hasOk) {
		...
		},
		{
			id: "pigeonhole",
			name: "归档",
			icon : "icon iconfont bd bd-folder-o",
			isHide: false,
			click: function () {
				let selection = this.$refs.list.selection;
				if (selection.length <= 0) {
				...
				this.$confirm('您确定归档选定的记录行（流程文档实例）吗? \n' +titles , '归档', {
		        	...
		        	this.post(`${this.BASEURL}/instance/flow/flowinstancemonitor/archive.do`, params, (isSuccess, res) => {
	        	  		if (res.hasOk) {
	        	  			this.$refs.list.refresh();
