{
	...
	buttons: [{
			...
			click: function() {
        	    ...
        	    this.addressBook({
    				...
    				this.$confirm('您确定将选定的待办转办至[' + data.map(d => d.name).join(',') + ']吗?' , '转办', {
    		        	...
    		        	this.post(`${this.BASEURL}/workmaintain/workmaintainsave/transfer.do`, params, (isSuccess, res) => {
    		        		if (res.hasOk) {
		...
		},
		{
			...
			click : function () {
				...
				this.$confirm('您确定将选定的记录行转为已处理吗?' , '转为已处理', {
		        	...
		        	this.post(`${this.BASEURL}/workmaintain/workmaintainsave/trunk.do`, params, (isSuccess, res) => {
	        	  		if (res.hasOk) {
	        	  			this.$refs.list.refresh();
	        	  			this.alert('操作成功', 'success')
	        	  		} else {
		...
		},
		{
			id: "transforToUndo",
			name: "转为未处理",
			icon : "icon iconfont bd-redo-square",
			isHide: false,
			click : function () {
				let selection = this.$refs.list.selection;
				if (selection.length <= 0) {
