{
	// 前端写法
        ...
        },
		{
			...
			click : function () {
				...
				this.$confirm('您确定启动选定的记录行吗?' , '启动', {
		        	...
		        	this.post(`${this.BASEURL}/quartz/qwtemplatesave/batchStart.do`, params, (isSuccess, res) => {
	        	  		if (res.hasOk) {
		...
		},
		{
			id: "stop",
			name: "停止",
			icon : "icon iconfont bd-times-circle-o",
			isHide: false,
			click : function () {
				let selection = this.$refs.list.selection;
				if (selection.length <= 0) {
				...
				this.$confirm('您确定停止选定的记录行吗?' , '停止', {
		        	...
		        	this.post(`${this.BASEURL}/quartz/qwtemplatesave/batchStop.do`, params, (isSuccess, res) => {
	        	  		if (res.hasOk) {
	        	  			this.$refs.list.refresh();
		...
		},
		{
			id: "execute",
			name: "立即执行",
			icon : "icon iconfont bd-exchange",
			isHide: false,
			click : function () {
				let selection = this.$refs.list.selection;
				if (selection.length == 1) {
					this.$confirm('您确定立即执行选定的记录行吗?' , '执行', {
			        	...
			        	this.post(`${this.BASEURL}/quartz/qwtemplatesave/startQwJobIgnoredTime.do`, params, (isSuccess, res) => {
		        	  		if (res.hasOk) {
		        	  			this.$refs.list.refresh();
