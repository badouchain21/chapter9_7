<template>
    <div class="m-base-form padding defaultBg"  v-fullPageHeight>
        <el-row :gutter="10" class="h-per-100">
            <el-col :span="12" class="h-per-100">
                <div class="s-block">
                    <div class="form-title noBorder">
                            ...
                            <div class="historyList">
                                <template v-if="historyList.length !== 0">
                                    <div 
                                        ...
                                        <!-- <el-button plain type="primary" size="mini" @click="useHistoryAvatar(i)">使用</el-button> -->
                                    </div>
                                </template>
                                <template v-else>
                                    <div class="noData">
                                        <bd-icon name="noData" size='large'></bd-icon><br/>
                                        暂无数据
                                    </div>
                                </template>
                            </div>
...
<script>
    ...
    import { getUserHistoryAvatarList } from '@/api/frame/user'
    import GlobalConst from '@/service/global-const'
    import { getAttach } from '@/api/frame/index'
    export default {
        components: {},
        directives: {
            fullPageHeight: {
                inserted: function (el, binding) {
                    ...
                    let historyElement = findByClassName(el, 's-history')
        ...
        data () { // 定义页面变量
            return {
                ...
                historyList: [],
                btnList: [
                    { name: '保存', icon: '#bd-save-o', click: this.save, isLoad: false}
                ]
            }
        },
        computed: {
        ...
        methods: { 
            ...
            filePreview (e) {
                ...
                if ((chooseFile.name+"").length > 64) {
                    this.$message({
                        type: 'warning',
                        message: '文件名不能超过64个字符!'
                    })
                    return
                }
            ...
            useHistoryAvatar ({id}) {
                this.attach = null
                this.attachId = id
                this.selectImgSrc = getAttach(id)
            },
            // 保存头像
            save (btnObj) {
                ...
                this.postFile(url, { attach: this.attach, photoAttachId: this.attachId }, (isSuccess, res) => {
                    if (res.hasOk) {
            ...
            initAvatar () {
                this.selectImgSrc = getAttach(this.avatar) || ''
            }
        },
        // 可访问当前this实例
        created () {
