 ...
 */
export function findParentByClassName (element, aimClassName) {
    // 初始化返回元素
    ...
    try {
        ...
        console.error(`使用函数findParentByClassName异常：${e}`)
    }
    return _element
}
 ...
 */
export function findByClassName (parentElement, aimClassName, isNextLevel = false) {
    // 设置父元素默认为document
 ...
 */
export function findFirstChild (parentElement) {
    // 设置父元素默认为document
    parentElement = parentElement || document
    // 获取父级元素下所有子元素-HTMLCollection类型
